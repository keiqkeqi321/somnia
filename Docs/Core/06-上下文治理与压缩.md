...
2. **Semantic Janitor** — 语义脱水（60% 自动触发阈值，带会话级低收益熔断）
...
flowchart TD
    ...
    D --> E{usage >= 60% and janitor armed?}
...
阈值：`SEMANTIC_JANITOR_TRIGGER_RATIO = 0.60`（上下文使用率 60%）

### 核心机制

当上下文使用率达到自动触发水位时，在 payload 构建阶段对历史工具结果执行语义脱水：
