...
class SystemPromptBuilder:
    ...
    def build_prompt_bundle(self, actor: str = "lead", role: str = "lead coding agent", session=None) -> PromptBundle:
        ...
        workflow_guidance = (
            ...
            "sequencing only matters when one call's input depends on another's result. Multiple Explore `subagent` "
            "calls in one turn also run in parallel; general-purpose subagent calls stay serial.\n"
        ...
        lead_guidance = (
            ...
            "A subagent may return a structured status of interrupted/truncated/failed instead of a summary; a "
            "truncated result may carry partial_summary with the findings so far — judge it before deciding whether "
            "to resume at all. When the user "
            "asks to continue (e.g. '继续'/'continue'/'go on'), RESUME such a subagent with resume_from=<activity_id> — it "
            "inherits the subagent's accumulated context at low cost; prompt is optional on resume, pass extra_prompt "
            "to refine the direction. "
            "If the user has changed direction, accept the partial result and move on instead. Do NOT restart the subagent "
