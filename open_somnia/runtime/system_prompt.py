...
class SystemPromptBuilder:
    ...
    def build_prompt_bundle(self, actor: str = "lead", role: str = "lead coding agent", session=None) -> PromptBundle:
        ...
        lead_guidance = (
            ...
            "A subagent may return a structured status of interrupted/truncated/failed instead of a summary. When the user "
            "asks to continue (e.g. '继续'/'continue'/'go on'), RESUME such a subagent with resume_from=<activity_id> — it "
            "inherits the subagent's accumulated context at low cost; you may pass extra_prompt to refine the direction. "
